...
void Game::Judge(bool direction, int value) {
	if(direction){
		m_player.savePos.x = m_player.shape.pos.x + WIDTH / m_line[stage].x * value;
		m_player.savePos.y = m_player.shape.pos.y;
	}
	else {
		m_player.savePos.x = m_player.shape.pos.x;
		m_player.savePos.y = m_player.shape.pos.y + HEIGHT / m_line[stage].y * value;
	}

	for (int i = 0; i < wallSize[stage]; i++) {
		if (m_wall[i].flag && m_wall[i].pos.x == m_player.savePos.x && m_wall[i].pos.y == m_player.savePos.y) {
			m_player.flag[MOVE] = false;
		}
	}

	for (int i = 0; i < boxSize[stage]; i++) {
		if (m_box[i].pos.x == m_player.savePos.x && m_box[i].pos.y == m_player.savePos.y) {
			if (direction) {
				m_box[i].savePos.x = m_box[i].pos.x + WIDTH / m_line[stage].x * value;
				m_box[i].savePos.y = m_box[i].pos.y;
			}
			else {
				m_box[i].savePos.x = m_box[i].pos.x;
				m_box[i].savePos.y = m_box[i].pos.y + HEIGHT / m_line[stage].y * value;
			}
			...
			for (int j = 0; j < wallSize[stage]; j++) {
				if (m_wall[j].flag && m_box[i].savePos.x == m_wall[j].pos.x && m_box[i].savePos.y == m_wall[j].pos.y) {
					m_player.flag[MOVE] = false;
				}
			}

			for (int j = 0; j < boxSize[stage]; j++) {
				if (i != j && m_box[i].savePos.x == m_box[j].pos.x && m_box[i].savePos.y == m_box[j].pos.y) {
					m_player.flag[MOVE] = false;
				}
			}

			if (m_box[i].savePos.x <= 0 || m_box[i].savePos.x >= WIDTH || m_box[i].savePos.y <= 0 || m_box[i].savePos.y >= HEIGHT) {
				m_player.flag[MOVE] = false;
			}
		}
	}

	if (m_player.flag[MOVE]) {
		if (direction)
			while ((value == OPPOSITION && m_player.shape.pos.x > m_player.savePos.x) ||
				value == NORMAL && m_player.shape.pos.x < m_player.savePos.x) {
				...
				if ((value == OPPOSITION && m_player.shape.pos.x <= m_player.savePos.x) ||
					value == NORMAL && m_player.shape.pos.x >= m_player.savePos.x) {
					m_player.shape.pos.x = m_player.savePos.x;
					...
					for (int i = 0; i < boxSize[stage]; i++)
						if (m_box[i].flag[PLAYERCOLLISION])
							m_box[i].pos.x = m_box[i].savePos.x;
				}
			...
			while ((value == OPPOSITION && m_player.shape.pos.y > m_player.savePos.y) ||
				value == NORMAL && m_player.shape.pos.y < m_player.savePos.y) {
				...
				if ((value == OPPOSITION && m_player.shape.pos.y <= m_player.savePos.y) ||
					value == NORMAL && m_player.shape.pos.y >= m_player.savePos.y) {
					m_player.shape.pos.y = m_player.savePos.y;
					...
					for (int i = 0; i < boxSize[stage]; i++)
						if (m_box[i].flag[PLAYERCOLLISION])
							m_box[i].pos.y = m_box[i].savePos.y;
				}
